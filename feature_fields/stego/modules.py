...
import feature_fields.dino.vision_transformer as vits
...
class DinoFeaturizer(nn.Module):
    ...
    def __init__(self, dim, patch_size, feat_type, model_type, pretrained_weights, use_dropout, projection_type, **_):
        super().__init__()
        # self.cfg = cfg
        self.dim = dim
        ...
        self.model = vits.__dict__[model_type](
        ...
        if model_type == "vit_small" and patch_size == 16:
            url = "dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth"
        elif model_type == "vit_small" and patch_size == 8:
            url = "dino_deitsmall8_300ep_pretrain/dino_deitsmall8_300ep_pretrain.pth"
        elif model_type == "vit_base" and patch_size == 16:
            url = "dino_vitbase16_pretrain/dino_vitbase16_pretrain.pth"
        elif model_type == "vit_base" and patch_size == 8:
            url = "dino_vitbase8_pretrain/dino_vitbase8_pretrain.pth"
        else:
            raise ValueError("Unknown model_arch and patch size")
...
def sample_at_coord(t: torch.Tensor, coords: torch.Tensor):
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def forward(self,
        ...
        feats = sample_at_coord(orig_feats, coords1)
        code = sample_at_coord(orig_code, coords1)

        feats_pos = sample_at_coord(orig_feats_pos, coords2)
        code_pos = sample_at_coord(orig_code_pos, coords2)
        ...
        for i in range(self.cfg.neg_samples):
            ...
            feats_neg = sample_at_coord(orig_feats[perm_neg], coords2)
            code_neg = sample_at_coord(orig_code[perm_neg], coords2)
...
class Decoder(nn.Module):
    def __init__(self, code_channels, feat_channels):
        super().__init__()
        self.linear = nn.Conv2d(code_channels, feat_channels, (1, 1))
        self.nonlinear = nn.Sequential(
            nn.Conv2d(code_channels, code_channels, (1, 1)),
            nn.ReLU(),
            nn.Conv2d(code_channels, code_channels, (1, 1)),
            nn.ReLU(),
            nn.Conv2d(code_channels, feat_channels, (1, 1)))
